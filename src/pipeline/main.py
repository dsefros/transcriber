...
def analyze_with_model(segments: list) -> str:
    ...
    # Системный промпт (без изменений — критичен для предметной области)
    system_prompt = f"""
...
"""
    ...
    # Формирование финального промпта (для llama-cpp добавляем системную роль вручную)
    if profile.backend == "llama_cpp":
        # llama-cpp не поддерживает отдельную системную роль — встраиваем в промпт
        full_prompt = f"Ты — эксперт по анализу технических встреч в компании Соммерс.\n{system_prompt.strip()}\n\nДиалог для анализа:\n{dialogue}"
    else:
        # Ollama поддерживает системную роль — передаём как есть
        full_prompt = f"Диалог для анализа:\n{dialogue}"
        # Для Ollama системный промпт передаётся отдельно в messages (см. ниже)
...
def main(audio_file: str, device: str = "cuda"):
    ...
    # Загрузка профиля для отображения в логе
    from src.config.models import get_models_config
    models_cfg = get_models_config()
    profile = models_cfg.get_active_profile()
    print(f"🧠 Модель анализа: {profile.key} ({profile.backend})\n")
