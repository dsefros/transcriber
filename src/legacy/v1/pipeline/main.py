...
from src.legacy.v1.storage import (
    # Legacy-only compatibility facade over canonical infrastructure storage.
    Fragment,
    Meeting,
    Speaker,
...
def transcribe_and_diarize(audio_path: str, device: str = "cuda"):
    ...
    from src.infrastructure.transcription.whisperx_runtime import (
        transcribe_and_diarize as runtime_transcribe_and_diarize,
    )
...
def save_to_databases(
    session, qdrant_client, filename, segments, analysis_md, original_audio_path
):
    ...
    try:
        ...
        meeting = (
            session.query(Meeting).filter(Meeting.filename == filename).one_or_none()
        )
        ...
        if meeting:
            ...
            session.query(Fragment).filter(Fragment.meeting_id == meeting.id).delete()
        ...
        for seg in segments:
            ...
            if name not in speaker_cache:
                speaker = (
                    session.query(Speaker).filter_by(external_id=name).one_or_none()
                )
                if not speaker:
                    speaker = Speaker(
                        external_id=name,
        ...
        for i, seg in enumerate(segments):
            ...
            fragment = Fragment(
                ...
                technical_terms=json.dumps(extract_technical_terms(seg["text"])),
...
def main(
    ...
    try:
        ...
        if is_reanalyze_mode:
            segments, orig_filename, audio_hash, duration = load_segments_from_json(
                json_file
            )
            session = None
            qdrant_client = None
        ...
        # --- AUDIO ---
            ...
            print(
                f"[DEBUG] Используем предрасчитанные сегменты: {precomputed_segments_path}"
            )
